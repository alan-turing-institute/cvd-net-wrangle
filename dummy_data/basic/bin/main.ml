...
let generate_date (years:int) : Unix.tm = 
    {
        Unix.tm_sec = 0;
        tm_min = 0;
        tm_hour = 0;
        tm_mday = 1;
        tm_mon = 0;    (* January = 0 *)
        tm_year = years; (* 2000 - 1900 = 100 *)
        tm_wday = 0; tm_yday = 0; tm_isdst = false
    } 

let generate_patients (n:int) : patient list =
    (* let study_start_date = generate_date 25 in
    let study_end_date = generate_date 50 in *)
    let youngest_dob = generate_date 100 in
    let oldest_dob = generate_date 60 in
    List.init n (fun i -> 
        ...
        let date_of_referral = advance_date date_of_birth (int_of_float @@ sample_age_referral ()) in
